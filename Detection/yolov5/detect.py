...
@torch.no_grad()
def run_eval_human(model=None,  # model.pt path(s)
        img_numpy=None,  # file/dir/URL/glob, 0 for webcam
    ...
    # Padded
    img = letterbox(img_numpy, imgsz, stride=stride, auto=True)[0]
    # Convert
    ...
    # Run inference
    model(torch.zeros(1, 3, imgsz, imgsz).to('cuda:0').type_as(next(model.parameters())))  # run once
    ...
    for i, det in enumerate(pred):  # per image
      seen += 1
      gn = torch.tensor(img_numpy.shape)[[1, 0, 1, 0]]  # normalization gain whwh
      imc = img_numpy.copy() if save_crop else img_numpy  # for save_crop
      annotator = Annotator(img_numpy, line_width=line_thickness, example=str(names))

      # If have detections      
      if len(det):
        # Rescale boxes from img_size to im0 size
        det[:, :4] = scale_coords(img.shape[2:], det[:, :4], img_numpy.shape).round()
