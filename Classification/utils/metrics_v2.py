...
def accuracy(output, target, dataset: ClothesClassificationDataset):
    ...
    return type_acc # torch.Tensor(1), torch.Tensor(color_len)


def F1_score(output, target, dataset: ClothesClassificationDataset):
    with torch.no_grad():
        color_target = target[:, dataset.type_len:]  # torch.Tensor(batch, dataset.color_len)
        color_output = output[1]
        color_output = (color_output > 0.5).type(torch.int) # torch.Tensor(batch, dataset.color_len)

        # compute F1 score for color
        ...
        F1_matching = torch.tensor([0] * dataset.color_len)

        # cal F1
        for i in range(dataset.color_len):
            tp = torch.sum(confusion_vector[:, i] == 1) # torch.Tensor
            fp = torch.sum(torch.Tensor.isinf(confusion_vector[:, i])) # torch.Tensor
            fn = torch.sum(confusion_vector[:, i] == 0) # torch.Tensor
            F1_matching[i] = tp/(tp + 1/2 * (fp + fn))
